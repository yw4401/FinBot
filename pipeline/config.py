...
FINE_TUNE_QA_ARTICLES = "fine-tune-qa-articles.parquet"
FINE_TUNE_QA_COREF = "fine-tune-qa-coref.parquet"
FINE_TUNE_QA_ARTICLES_COREF = "fine-tune-qart-coref.parquet"
...
TOPIC_EMBED_TOP_THRESHOLD = 0.51
...
QA_AUG_SYSTEM = "You are a helpful AI assistant that will come up with a good question that can be answered by the given text, " \
                "and a good question that cannot be answered by the text. " \
                "The answer to the answerable question must be from the information in the provided text. " \
                "Both questions must be something that a retail investor who hasn't seen the given text is likely going to ask. " \
                "The output should be formatted as:\n" + \
                """Response:
REASON (Unanswerable): reason why a retail investor would ask the question that cannot be answered.
UNANSWERABLE: reason why the question cannot be answered with only information from the text.
...
REASON (answerable): reason why a retail investor would ask the question that can be answered.
ANSWER: concise answer to the question from the text at most one paragraph long. the answer should not include elements such as bullet points.
QUESTION (answerable): the answerable question.

Follow the format. Begin.
"""
QA_AUG_USER = "{input_text}"
QA_PARSE_SYSTEM = "You are a helpful AI assistant that will convert natural language into a machine parsable format. {format_instructions}"
QA_PARSE_USER = "{text}"
QA_PARSE_EXAMPLE = [{"user": """REASON (Unanswerable): Retail investors would want to estimate the potential financial repercussions of the lawsuit on Anthropic in order to assess the company's financial stability and investment prospects.
UNANSWERABLE: The text only mention that a lawsuit occurred, but does not mention any specific consequences of the lawsuit
QUESTION: What potential financial damages or penalties is Anthropic facing as a result of the lawsuit filed by Universal Music and other plaintiffs?
...
ANSWER: Anthropic is being sued by music publishers including Universal Music, Concord, and ABKCO over copyrighted lyrics generated by Claude.
QUESTION: What are some legal risks that Anthropic is currently facing?""",
                              "assistant": """{"answer": "Anthropic is being sued by music publishers including Universal Music, Concord, and ABKCO over copyrighted lyrics generated by Claude.", "answerable": "What are some legal risks that Anthropic is currently facing?", "unanswerable": "What potential financial damages or penalties is Anthropic facing as a result of the lawsuit filed by Universal Music and other plaintiffs?"}"""}]
